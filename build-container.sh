...
test_container() {
    ...
    PHP_VERSION=$($CONTAINER_CMD run --rm ${FULL_IMAGE}:${OPENEMR_VERSION} php -v | head -n 1)
    print_success "PHP Version: $PHP_VERSION"
    
    # Test 2: Check nginx version
    print_info "Test 2: Checking nginx version..."
    NGINX_VERSION=$($CONTAINER_CMD run --rm ${FULL_IMAGE}:${OPENEMR_VERSION} nginx -v 2>&1)
    print_success "nginx Version: $NGINX_VERSION"
    
    # Test 3: Check PHP modules
    print_info "Test 3: Checking required PHP modules..."
    REQUIRED_MODULES=("gd" "mysqlnd" "xml" "mbstring" "zip" "curl" "opcache" "soap" "ldap")
    
    for module in "${REQUIRED_MODULES[@]}"; do
        if $CONTAINER_CMD run --rm ${FULL_IMAGE}:${OPENEMR_VERSION} php -m | grep -q "^${module}$"; then
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            --help)
                ...
                echo "  --version VER   Specify OpenEMR version (default: 7.0.5)"
